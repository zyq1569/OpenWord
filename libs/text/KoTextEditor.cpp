...
///TODO factor out the changeTracking charFormat setting from all individual slots to a public slot,
///  which will be available for external commands (TextShape)

///The BlockFormatVisitor and CharFormatVisitor are used when
/// a property needs to be modified relative to its current value
///  (which could be different over the selection).
///  For example: increase indentation by 10pt.
///The BlockFormatVisitor is also used for the
/// change tracking of a blockFormat.
/// The changeTracker stores the information about the
/// changeId in the charFormat. The BlockFormatVisitor ensures
///  that thd changeId is set on the whole block (even if only a part of the block is actually selected).
...
void KoTextEditor::registerTrackedChange(QTextCursor &selection, KoGenChange::Type changeType, const KUndo2MagicString &title, QTextFormat& format, QTextFormat& prevFormat, bool applyToWholeBlock)
{
    ...
    else
    {
        if (changeType != KoGenChange::DeleteChange)
        {
            ...
            ///TODO: this whole thing actually needs to be done like a visitor.
            /// If the selection contains several change regions,
            /// the parenting needs to be individualised.
            switch (changeType)
            {
...
KoAnnotation *KoTextEditor::addAnnotation(KoShape *annotationShape)
{
    ...
    ///FIXME: I need the name, a unique name, we can set selected text
    /// as annotation name or use createUniqueAnnotationName function
    /// to do it for us.
