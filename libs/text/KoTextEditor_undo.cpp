...
/// This slot is called when the KoTextEditor receives the signal undoCommandAdded() from QTextDocument.
/// A generic UndoTextCommand is used to match the QTextDocument's internal undo command.
/// This command calls QTextDocument::undo() or QTextDocument::redo() respectively, which triggers the undo redo actions on the document.
///In order to allow nesting commands, we maintain a stack of commands.
/// The top command will be the parent of our auto generated UndoTextCommands.
///Depending on the case, we might create a command which will serve as head command.
/// This is pushed to our commandStack and eventually to the application's stack.
void KoTextEditor::Private::documentCommandAdded()
{
    ...
    public:
        UndoTextCommand(QTextDocument *document, KoTextEditor::Private *p, KUndo2Command *parent = 0)
            : KUndo2Command(kundo2_i18n("Text"), parent),  m_document(document), m_p(p)
        {
...
/// This method is used to push a complete KUndo2Command on the KoTextEditor.
/// This command will be pushed on the application's stack if needed.
/// The logic allows to push several commands which are then going to be nested,
/// provided these children are pushed from within the redo method of their parent.
...
/// Important note:
/// ***
/// The framework does not allow to push a complete KUndo2Command (through KoTextEditor::addCommand)
/// from within an EditBlock. Doing so will lead in the best case to several undo/redo commands on
/// the application's stack instead of one, in the worst case to an out of sync application's stack.
/// ***
KUndo2Command *KoTextEditor::beginEditBlock(const KUndo2MagicString &title)
{
    ...
    {
        /// We are not in a custom macro command. So we first need to update the KoTextEditor's state to Custom.
        /// Additionally, if the commandStack is empty, we need to create a master headCommand for our macro and push it on the stack.
    ...
    ///QTextDocument sends the undoCommandAdded signal at the end of the QTextCursor edit block.
    /// Since we want our master headCommand to parent the signal induced UndoTextCommands, we should not call QTextCursor::beginEditBlock for the headCommand.
...
void KoTextEditor::endEditBlock()
{
    ...
    ///Only the self created master headCommand (see beginEditBlock) is left on the commandStack,
    /// we need to decrease the customCommandCount counter that we increased on creation.
